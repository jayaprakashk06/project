...
def render_overview(df: pd.DataFrame) -> None:
    st.subheader("🏙️ City Risk Overview")
    left, right = st.columns([1.1, 1.9])
    with left:
        district_breakdown = df["district"].value_counts().head(8)
        st.caption("Top districts by incident volume")
        st.bar_chart(district_breakdown)
    with right:
        if PLOTLY_AVAILABLE:
            trend = df.groupby(["month", "crime_type"], as_index=False).size().rename(columns={"size": "count"})
            fig = px.line(
                trend,
                x="month",
                y="count",
            ...
            st.plotly_chart(fig, width='stretch', key='overview_monthly_trend')
        else:
            month_count = df.groupby("month", as_index=False).size().rename(columns={"size": "count"})
            st.line_chart(month_count.set_index("month")["count"])
...
def render_prediction_panel(
    ...
    st.markdown(
        '<span class="pill">Realtime: {}</span><span class="pill">Model: RandomForest</span>'.format(
            "ON" if realtime_mode else "OFF"
        ),
        unsafe_allow_html=True,
    )
    ...
    if predict_clicked:
        ...
        risk_level = "High" if prob_high >= 0.65 else ("Medium" if prob_high >= 0.35 else "Low")
        k1, k2 = st.columns(2)
        k1.metric("Predicted High-Risk Probability", f"{prob_high:.2%}")
    ...
    st.markdown("---")
    st.subheader("📈 Linked District Dashboard")
    render_dashboard(df, district)


def render_data_page(raw_df: pd.DataFrame, df: pd.DataFrame) -> None:
    st.subheader("🧪 Data Quality & Preview")
    c1, c2, c3 = st.columns(3)
    c1.metric("Rows (raw)", f"{len(raw_df):,}")
    c2.metric("Rows (clean)", f"{len(df):,}")
    c3.metric("Removed rows", f"{max(len(raw_df) - len(df), 0):,}")

    st.caption("Validated columns")
...
def main() -> None:
    ...
    page = st.sidebar.radio(
        "Go to section",
        ["🏠 Overview", "🎯 Prediction Studio", "📊 Analytics", "🧪 Data Quality"],
    ...
    if page == "🏠 Overview":
        ...
        render_prediction_panel(df, model, realtime_mode)
    elif page == "📊 Analytics":
        render_dashboard(df, top_district)
    else:
        render_data_page(raw_df, df)
